...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/nw-tomfoolery.zip'
        },
        files: [
          {
            expand: true,
        ...
        files: [
          {
            ...
            dest: '<%= config.dist %>/nw-tomfoolery.app'
          }
        ]
      },
      zipToApp: {
  ...
  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/nw-tomfoolery.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/nw-tomfoolery.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/nw-tomfoolery.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/nw-tomfoolery.app/Contents/MacOS/node-webkit', '555');
  });
